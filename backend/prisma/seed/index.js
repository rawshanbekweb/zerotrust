...
async function main() {
    ...
    const permissions = await Promise.all(resources.flatMap((resource) => actions.map((action) => prisma.permission.upsert({
    ...
    const adminPerms = permissions.filter((p) => !(p.resource === 'settings' && p.action === 'delete'));
    ...
    const analystPerms = permissions.filter((p) => p.action === 'read' ||
    ...
    // ─── USERS ─────────────────────────────────────────────────
    const adminPassword = await argon2_1.default.hash('Admin@ZeroTrust2024');
    const userPassword = await argon2_1.default.hash('User@ZeroTrust2024');
    const users = [
        {
        ...
        {
            email: 's.jalgas@nationalbank.uz',
            username: 'jalgas_3',
            firstName: 'Jalg\'as',
            lastName: 'Sarsenbayev 3',
            department: 'Finance',
    ...
    // ─── DEVICES ──────────────────────────────────────────────────
    const deviceTypes = ['DESKTOP', 'MOBILE', 'LAPTOP'];
